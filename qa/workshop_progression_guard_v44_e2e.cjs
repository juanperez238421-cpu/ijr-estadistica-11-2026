...
async function waitForServer() {
  for (let i = 0; i < 50; i += 1) {
    try { const response = await fetch(`${ORIGIN}/python/workshop.html?topic=statistics`); if (response.ok) return; } catch {}
...
async function run() {
  ...
  try {
    ...
    const submits = [];
    await context.route('**/rest/v1/rpc/python_hub_resume_v1', route => route.fulfill({ status:200, contentType:'application/json', body:JSON.stringify({ snapshot }) }));
    await context.route('**/rest/v1/rpc/python_hub_submit_v1', async route => {
      ...
      recalc(snapshot);
      await route.fulfill({ status:200, contentType:'application/json', body:JSON.stringify({ correct, message:correct?'Correct. Progress saved.':'Not correct yet. Expected output not confirmed.', snapshot }) });
    });
    ...
    await page.waitForFunction(() => document.getElementById('nextButton')?.disabled === true, null, { timeout:3000 });
    ...
    const kickerAfterBypass = await page.locator('#problemKicker').innerText();
    if (!/STAGE\s+2\s+OF/i.test(kickerAfterBypass)) throw new Error('Client-side bypass advanced past an unvalidated Stage 2.');
    if (!(await page.locator('#nextButton').isDisabled())) throw new Error('Guard did not restore the Stage 2 lock after bypass attempt.');

    console.log('WORKSHOP PROGRESSION V44 E2E PASS wrong_output=LOCKED exact_backend_output=UNLOCKED stage2=RELOCKED bypass=BLOCKED');
  } finally {
...
run().then(() => process.exit(0)).catch(error => { console.error(error.stack || error); process.exit(1); });
