(() => {
  ...
  function topics() {
    ...
    students().forEach((student) => (student.topics || []).forEach((topic) => {
      if (!bySlug.has(topic.slug)) bySlug.set(topic.slug, topicMeta(topic.slug, topic));
    }));
    return [...bySlug.values()].sort((a, b) => a.sequence - b.sequence);
  }
  ...
  function studentActivity(student) {
    const rows = (student.topics || []).map((topic) => {
      const meta = topicMeta(topic.slug, topic);
      const progress = itemProgress(topic);
      return { topic, meta, progress };
    });
    const lastActivity = latestDate(rows.map((row) => row.progress.lastActivity));
    const todayRows = rows.filter((row) => row.progress.lastActivity && isTodayBogota(row.progress.lastActivity));
    return { rows, lastActivity, todayRows };
  }

  function status(topic, selectedMeta) {
    const last = itemProgress(topic).lastActivity;
    const w = selectedMeta?.sequence || '—';
    if (!last) return `<span class="sp2-status none">Sin actividad en W${esc(w)}</span>`;
    const age = Date.now() - last.getTime();
    if (age <= 8 * 60 * 1000) return `<span class="sp2-status active">Activo en W${esc(w)}</span>`;
    if (isTodayBogota(last)) return `<span class="sp2-status today">Trabajó hoy en W${esc(w)}</span>`;
    return `<span class="sp2-status historical">Histórico en W${esc(w)}</span>`;
  }

  function dailyActivityHtml(student) {
    const d = studentActivity(student);
    if (d.todayRows.length) {
      const workshops = d.todayRows.map((row) => `W${row.meta.sequence}`).join(' · ');
      return `<small class="sp2-daily today">Hoy (todos): ${esc(workshops)} · último ${esc(fmtTime(d.lastActivity))}</small>`;
    }
    if (d.lastActivity) return `<small class="sp2-daily">Última actividad general: ${esc(fmtTime(d.lastActivity))}</small>`;
  ...
  function renderDataQuality() {
    ...
    const preview = scoped.slice(0, 12).map((row) => {
      const active = Array.isArray(row.active_topics) ? row.active_topics.map((slug) => {
        const meta = topicMeta(slug);
        return `W${meta.sequence}`;
      }).join(', ') : '—';
      return `<li><strong>${esc(row.group_code || '—')}</strong> · ${esc(row.institutional_email || row.display_name || 'registro sin identidad')} · ${esc(active)} · ${esc(fmtTime(row.last_response_at))}</li>`;
    }).join('');
    const extra = scoped.length > 12 ? `<li>+${scoped.length - 12} registros adicionales</li>` : '';
    el.className = 'sp2-data-quality';
    el.innerHTML = `<details><summary>⚠ Auditoría de identidad: ${scoped.length} registro(s) con respuestas hoy todavía no están vinculados al listado oficial.</summary><p>No se descartan: se mantienen separados para evitar atribuir actividad al estudiante equivocado. Al confirmar identidad deben vincularse al roster.</p><ul>${preview}${extra}</ul></details>`;
  }
  ...
  function render() {
    ...
    $('studentProgressSummaryV2').innerHTML = selected
      ? `<div><span>Grupo</span><strong>${esc(state.group || 'Todos')}</strong></div><div><span>Workshop</span><strong>${esc(selectedMeta.sequence)}</strong></div><div><span>Ejercicios</span><strong>${esc(totalExercises)}</strong></div><div><span>Estudiantes</span><strong>${esc(groupStudents.length)}</strong></div><div><span>Actividad en W${esc(selectedMeta.sequence)}</span><strong>${esc(withActivity)}</strong></div><div><span>Actividad hoy · cualquier W</span><strong>${esc(todayAnyWorkshop)}</strong></div><div><span>Validados en W${esc(selectedMeta.sequence)}</span><strong>${esc(validated)}</strong></div>`
      : '';
