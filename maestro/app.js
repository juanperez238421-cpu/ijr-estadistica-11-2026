  ...
  function render(){
    ...
    $('gradeBody').innerHTML=combined.map(r=>{
      ...
      return `<tr>
        ...
        <td>${completed}/${total}</td><td class="grade">${fmt(visibleGrade(ar))}</td><td>${esc(support)}</td><td>${esc(ar?.restriction_events??0)}</td><td>${fmtTime(ar?.last_activity_at)}</td>
      </tr>`;
    }).join('')||'<tr><td colspan="11">Sin resultados para el filtro.</td></tr>';
