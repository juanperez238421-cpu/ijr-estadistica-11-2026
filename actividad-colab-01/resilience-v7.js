(() => {
  ...
  // Wrap the Supabase client before app.js creates it. Safe reads are retried.
  // Mutations are never blindly repeated: after a network error we resume the
  // server snapshot and detect whether the first request was already committed.
  if (window.supabase?.createClient) {
    ...
    window.supabase.createClient = (...args) => {
      ...
      async function raw(name,params={}) {
        return await nativeRpc(name,params);
      }

      async function resume(params,attempts=3) {
        let last;
        for(let i=0;i<attempts;i++){
          last=await raw('student_learning_activity_resume',{
            p_attempt_id:params.p_attempt_id,
            p_attempt_token:params.p_attempt_token
          });
      ...
      client.rpc = async (name,params={}) => {
        ...
        if(name==='student_learning_activity_submit'){
          ...
          if(committed)return {data:{
            correct:!!next.correct,
            awarded_points:Number(next.awarded_points||0),
            wrong_attempts:Number(next.wrong_attempts||0),
            stage_potential:Number(next.stage_potential??next.awarded_points??0),
            snapshot:after,
        ...
        if(name==='student_learning_activity_use_help'){
          ...
          if(afterUsed>beforeUsed)return {data:{
            help_level:Number(next.help_count||1),
            help_tokens_used:afterUsed,
            help_tokens_remaining:Number(after.help_tokens_remaining||0),
            stage_potential:Number(next.stage_potential??0),
            snapshot:after,
            recovered_after_network_error:true
          },error:null};
        }
        ...
        if(name==='student_learning_activity_reveal_solution'&&next.completed&&next.completion_mode==='revealed'){
          return {data:{
            expected_answer:next.expected_answer??next.latest_answer??'',
            awarded_points:Number(next.awarded_points||0),
            snapshot:after,
            recovered_after_network_error:true
          },error:null};
