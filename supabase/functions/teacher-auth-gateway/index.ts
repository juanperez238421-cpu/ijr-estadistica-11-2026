...
const allowedOperations: Record<string, { rpc: string; args: string[] }> = {
  ...
  statistics_update_registration: {
    rpc: "teacher_learning_activity_update_registration_v10",
    args: ["p_attempt_id", "p_group_code", "p_student_emails"],
  },
  statistics_delete_registration: {
    rpc: "teacher_learning_activity_delete_v10",
    args: ["p_attempt_id"],
  },
  assessment_dashboard: {
    rpc: "teacher_dashboard_snapshot",
    args: ["p_assessment_slug"],
  },
  assessment_detail: { rpc: "teacher_attempt_detail", args: ["p_attempt_id"] },
  assessment_action: {
    rpc: "teacher_code_action",
    args: ["p_assessment_slug", "p_action", "p_attempt_id"],
  },
  assessment_delete: {
    rpc: "teacher_delete_attempt",
    args: ["p_assessment_slug", "p_attempt_id", "p_confirmation"],
  },
  ...
  seminar_oop_colab_dashboard: {
    rpc: "teacher_seminar_oop_colab_dashboard_v1",
    args: [],
  },
  seminar_oop_colab_detail: {
    rpc: "teacher_seminar_oop_colab_detail_v1",
    args: ["p_attempt_id"],
  },
  seminar_oop_colab_delete: {
    rpc: "teacher_seminar_oop_colab_delete_v1",
    args: ["p_attempt_id"],
  },
};
...
Deno.serve(async (req: Request) => {
  ...
  const audit = async (decision: "allow" | "deny" | "error", reason: string) => {
    await adminClient.from("teacher_auth_gateway_audit").insert({
      auth_user_id: userId,
      auth_session_id: typeof claims.session_id === "string" ? claims.session_id : null,
      operation,
      decision,
  ...
  try {
    ...
    if (!route) {
      await audit("deny", "operation_not_allowed");
      return json(origin, 404, { error: "not_found" });
    }

    const { data: userData, error: userError } = await userClient.auth.getUser(token);
    if (userError || !userData.user) {
      await audit("deny", "invalid_user_session");
      return json(origin, 404, { error: "not_found" });
    }
    ...
    if (aalError || aal?.currentLevel !== "aal2") {
      await audit("deny", "aal2_required");
      return json(origin, 403, { error: "mfa_required" });
    }
    ...
    if (profileError || !profile) {
      await audit("deny", "teacher_role_required");
      return json(origin, 404, { error: "not_found" });
    }

    const supplied = payload?.args && typeof payload.args === "object" ? payload.args : {};
    const rpcArgs: Record<string, unknown> = { p_teacher_token: "" };
    for (const key of route.args) rpcArgs[key] = supplied[key];

    const { data, error } = await userClient.rpc(route.rpc, rpcArgs);
    if (error) {
      await audit("error", `${route.rpc}:${error.code ?? "rpc_error"}`);
      return json(origin, 400, { error: "operation_failed" });
    }
