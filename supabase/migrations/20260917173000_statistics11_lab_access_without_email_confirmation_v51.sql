...
begin
  ...
  if v_account.id is not null and v_account.group_code <> v_group then
    raise exception 'This institutional email belongs to group %, not %', v_account.group_code, v_group;
  end if;

  v_registry_id := private.python_hub_resolve_roster_v26(v_email,v_group);

  if v_registry_id is null and v_account.id is null then
    raise exception 'This institutional email is not registered for Statistics 11 in group %', v_group;
  end if;
  ...
  if v_registry_id is not null then
    ...
    v_identity_status := 'verified_roster_lab';
  else
    v_display := coalesce(nullif(trim(v_account.display_name),''),v_email);
    v_identity_status := coalesce(nullif(v_account.identity_status,''),'verified_email_lab');
  end if;
